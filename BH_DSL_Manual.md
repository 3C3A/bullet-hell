...
명령어는 대소문자를 구분하지 않는 편이 좋다.  
실제로는 첫 토큰과 일부 하위 토큰을 소문자로 바꿔 비교한다.

### 2-4. 문자열

문자열 따옴표는 없다.  
즉, 파일 경로, 라벨 이름, 변수 이름에는 공백이 들어가면 안 된다.
...
* `sin`, `cos`, `tan`, `atan`, `atan2`는 **라디안 기준**이다.
...
* 두 값 중 작은 값과 큰 값을 기준으로 난수 대기
* 결과는 반올림되어 정수 틱으로 사용됨
...
* 무한 반복은 `break` 또는 스크립트 종료 명령이 없으면 계속 돈다.
...
* 플레이어 뒤쪽으로 가는 선택은 피한다
* 플레이어가 느리거나 위기 상황이면 더 안전한 거리로 움직이려 한다
* 보스 전투용 추적/자리잡기 행동에 적합하다
...
### 22-5. 줄 번호 점프는 명령 인덱스다

`jump 10`은 파일의 10번째 글자가 아니라,  
파싱된 명령 목록의 인덱스를 뜻한다.
...
* `loop -1`은 최대한 피한다
* `wait` 없이 무한 반복하지 않는다
* `shot circle 1000 ...` 같은 과도한 수는 피한다
