...
다음 이름은 기본으로 읽을 수 있다.  
코드에서는 일부 이름이 `enemy_x`, `player_x`처럼 써도 같은 값으로 해석되도록 정규화된다.

### 적 위치 / 상태
- `x`, `enemyx`, `enemy_x`, `posx`, `px`
- `y`, `enemyy`, `enemy_y`, `posy`, `py`
- `hp`
- `maxhp`
- `phase`
- `time`
